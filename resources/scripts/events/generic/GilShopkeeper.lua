...
function onTalk(target, player)
    ...
    player:play_scene(SCENE_GREETING, HIDE_HOTBAR, {0, 1})
end
...
function onYield(scene, results, player)
    ...
    if scene == SCENE_GREETING then
        ...
        player:play_scene(SCENE_SHOW_SHOP, NO_DEFAULT_CAMERA | HIDE_HOTBAR, buyback_list)
    elseif scene == SCENE_SHOW_SHOP then
        ...
        elseif #results == 0 then -- The player closed the shop window.
            player:play_scene(SCENE_SHOP_END, NO_DEFAULT_CAMERA | HIDE_HOTBAR, {})
        end
    else
        player:finish_event()
    end
