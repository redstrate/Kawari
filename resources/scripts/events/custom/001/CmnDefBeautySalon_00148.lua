...
function onYield(scene, results, player)
    if scene == 0 then
        ...
        if results[1] == 1 then
            player:play_scene(player.id, 00001, FADE_OUT | HIDE_UI | CONDITION_CUTSCENE, {0})
            return
        end
    elseif scene == 1 then
        player:play_scene(player.id, 00002, FADE_OUT | HIDE_UI | CONDITION_CUTSCENE, {0})
        return
    elseif scene == 2 then
        player:play_scene(player.id, 00003, FADE_OUT | HIDE_UI | CONDITION_CUTSCENE, {0})
        return
    end
    player:finish_event()
end
